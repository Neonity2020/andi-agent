...
Tests in `test/` mirror source modules, for example `test/agent.test.ts`. Architecture plans belong in `.plans/`. The `docs/` directory is the sandbox for testing the Agent's generation capabilities, including HTML, images, screenshots, and other generated artifacts. `todo-app/` is an example project generated by andi-agent using Agnes, not core runtime code. Never commit runtime state from `.andi-agent/`.
